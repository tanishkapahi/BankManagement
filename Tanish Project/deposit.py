...
class deposit:
    ...
    def verify(self):
        ...
        font=('Arial', 10, 'bold'))
            ...
            self.l11=Label(self.root, text='Deposit Amount', background='maroon', foreground='yellow',
        font=('Arial', 10, 'bold'))
            ...
            self.b2=Button(self.root, text='Deposit',command=self.deposit)
    ...
    def deposit(self):
        conn= pymysql.connect(host='localhost', user='root', password='root', db='dbbank')
        cursor= conn.cursor()
        dep= int(self.e11.get())
        bal= int(self.e10.get())
        newbalance=dep+bal
        print(newbalance)

        cursor.execute('update tbaccount set balance =%s',newbalance)
        messagebox.showinfo('Banking System', 'Amount Deposited Successfully')
        conn.commit()
